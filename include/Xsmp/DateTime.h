...
struct DateTime {
    ...
    constexpr explicit DateTime(::Smp::DateTime dateTime) :
            _value(dateTime) {
    }
    ...
    constexpr
    DateTime& operator+=(const ::Smp::Duration &d) noexcept {
        _value += d;
        return *this;
    }
    constexpr
    ...
    template<class Clock, class FromDuration>
    constexpr explicit operator std::chrono::time_point<Clock, FromDuration>() const noexcept {
...
private:
    /// ::Smp::DateTime is expressed in nanoseconds, relative to the reference time of
    /// 01.01.2000, 12:00, Modified Julian Date (MJD) 2000+0.5;
    /// epoch time is 01.01.1970, 00:00
    /// there are 946728000 seconds between epoch time and MJD2000 +0.5
    ...
    constexpr ::Smp::DateTime convert(
        ...
        return std::chrono::duration_cast<std::chrono::nanoseconds>(
                tp.time_since_epoch()).count() - _epochToMjd;
    }
};

static_assert(sizeof(::Smp::DateTime)==sizeof(::Xsmp::DateTime), "Size of ::Xsmp::DateTime shall be identical to ::Smp::DateTime");
static_assert(std::is_standard_layout_v<::Xsmp::DateTime>,"::Xsmp::DateTime shall be a standard layout class");
...
constexpr DateTime operator+(const DateTime &x,
    ...
    return DateTime { static_cast<::Smp::DateTime>(x)
