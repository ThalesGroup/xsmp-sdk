...
struct String {
    ...
    constexpr void remove_suffix(std::size_t length) noexcept {
        ...
        internalString[length > currentLength ? 0 : currentLength - length] =
                '\0';
    }

    [[nodiscard]] constexpr std::string_view substr(size_type pos = 0,
            size_type n = std::string_view::npos) const noexcept (false) {
        return std::string_view { internalString }.substr(pos, n);
    }

    [[nodiscard]] constexpr size_type find(std::string_view str, size_type pos =
            0) const noexcept {
    ...
    [[nodiscard]] constexpr size_type rfind(std::string_view str,
            size_type pos = std::string_view::npos) const noexcept {
        return std::string_view { internalString }.rfind(str, pos);
    }

    [[nodiscard]] constexpr size_type rfind(char c, size_type pos =
            std::string_view::npos) const noexcept {
    ...
    [[nodiscard]] constexpr size_type find_first_of(std::string_view str,
            size_type pos = 0) const noexcept {
        return std::string_view { internalString }.find_first_of(str, pos);
    }
    [[nodiscard]] constexpr size_type find_first_of(char c,
            size_type pos = 0) const noexcept {
        return std::string_view { internalString }.find_first_of(c, pos);
    }

    [[nodiscard]] constexpr size_type find_first_of(const char *str,
    ...
    [[nodiscard]] constexpr size_type find_last_of(std::string_view str,
            size_type pos = std::string_view::npos) const noexcept {
        return std::string_view { internalString }.find_last_of(str, pos);
    }

    [[nodiscard]] constexpr size_type find_last_of(char c, size_type pos =
            std::string_view::npos) const noexcept {
        return std::string_view { internalString }.find_last_of(c, pos);
    }

    [[nodiscard]] constexpr size_type find_last_of(const char *str,
            size_type pos, size_type n) const noexcept {
    ...
    [[nodiscard]] constexpr size_type find_first_not_of(std::string_view str,
            size_type pos = 0) const noexcept {
        return std::string_view { internalString }.find_first_not_of(str, pos);
    }

    [[nodiscard]] constexpr size_type find_first_not_of(char c, size_type pos =
            0) const noexcept {
        return std::string_view { internalString }.find_first_not_of(c, pos);
    }

    [[nodiscard]] constexpr size_type find_first_not_of(const char *str,
            size_type pos, size_type n) const noexcept {
        return std::string_view { internalString }.find_first_not_of(str, pos,
                n);
    }
    ...
    [[nodiscard]] constexpr size_type find_last_not_of(std::string_view str,
            size_type pos = std::string_view::npos) const noexcept {
        return std::string_view { internalString }.find_last_not_of(str, pos);
    }

    [[nodiscard]] constexpr size_type find_last_not_of(char c, size_type pos =
            std::string_view::npos) const noexcept {
        return std::string_view { internalString }.find_last_not_of(c, pos);
    }

    [[nodiscard]] constexpr size_type find_last_not_of(const char *str,
            size_type pos, size_type n) const noexcept {
...
template<std::size_t Nm>
std::istream&
operator>>(std::istream &is, String<Nm> &str) {
    std::string tmp;
    is >> tmp;
    str = tmp;
    return is;
