...
Simulator::Simulator(::Smp::String8 name, ::Smp::String8 description) :
        ...
        _holdImmediately { "HoldImmediately", "", this, [this] {
...
Simulator::~Simulator() {
    // Exit the simulation properly if the simulator is not already in exit or abort state
    ...
    while (!_libraries.empty()) {
        ...
        if ((*::Xsmp::GetSymbol<bool (*)(::Smp::ISimulator *simulator)>(handle,
            ...
            if (_logger) {
                _logger->Log(this,
                        ("Unable to unload " + name + " library.").c_str(),
...
void Simulator::Publish() {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Building) {
        if (_logger) {
            _logger->Log(this,
                    "Could not Publish the Simulation if simulator is not in Building state.",
...
void Simulator::Configure() {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Building) {
        if (_logger) {
            _logger->Log(this,
                    "Could not Configure the Simulation if simulator is not in Building state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        }
        return;
    }
...
void Simulator::Connect() {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Building) {
        if (_logger)
            _logger->Log(this,
                    "Could not Connect the Simulation if simulator is not in Building state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
...
void Simulator::Initialise() {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Standby
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Initialise the Simulation if simulator is not in Standby state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
...
void Simulator::Run(::Smp::Duration duration) {
    ...
    ::Xsmp::EntryPoint hold { "hold",
            "call simulator hold after the specified duration", this, [this] {
                this->Hold(false);
            } };
    if (_scheduler)
...
void Simulator::Hold(::Smp::Bool immediate) {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Executing
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Hold the Simulation if simulator is not in Executing state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
...
enum class PersistKind {
    PERSIST, COMPONENT, COMPOSITE, CONTAINER, FIELD
};
namespace {
...
void Simulator::Store(::Smp::String8 filename) {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Standby
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Store the Simulation if simulator is not in Standby state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
    EmitGlobalEvent(::Smp::Services::IEventManager::SMP_LeaveStandbyId);
...
void Simulator::Restore(::Smp::String8 filename) {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Standby
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Restore the Simulation if simulator is not in Standby state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
    EmitGlobalEvent(::Smp::Services::IEventManager::SMP_LeaveStandbyId);
...
void Simulator::Reconnect(::Smp::IComponent *root) {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Standby
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Reconnect the Simulation if simulator is not in Standby state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
    EmitGlobalEvent(::Smp::Services::IEventManager::SMP_LeaveStandbyId);
...
void Simulator::Exit() {
    ...
    if (_state != ::Smp::SimulatorStateKind::SSK_Standby
        ...
        if (_logger)
            _logger->Log(this,
                    "Could not Exit from Simulation while simulator is not in Standby state.",
                    ::Smp::Services::ILogger::LMK_Warning);
        return;
    }
...
void Simulator::AddService(::Smp::IService *service) {
    ...
    auto RegisterService = [service, this](auto *&value) {
        ...
        if ((value =
                ...
                && _logger)
            _logger->Log(this,
                    ("Service '" + ::Xsmp::Helper::TypeName(value)
                            + "' registered.").c_str(),
...
::Smp::IComponent* Simulator::CreateInstance(::Smp::Uuid uuid,
    ...
    return factory ?
            factory->CreateInstance(name, description, parent) : nullptr;
}
...
void Simulator::LoadLibrary(::Smp::String8 libraryPath) {
    ...
    if (!handle) {
        ...
        if (_logger)
            _logger->Log(this, error.c_str(),
                    ::Smp::Services::ILogger::LMK_Error);
        ::Xsmp::Exception::throwLibraryNotFound(this, libraryPath, error);
    }

    auto initialise = ::Xsmp::GetSymbol<
            bool (*)(::Smp::ISimulator *simulator,
        ...
        if (_logger)
            _logger->Log(this,
                    ("Library '" + std::string(libraryPath)
                            + "' successfully loaded.").c_str(),
                    ::Smp::Services::ILogger::LMK_Debug);
    }
    else {
        std::string msg = std::string("Initialise() of library '") + libraryPath
                + "' failed.";
        if (_logger)
            _logger->Log(this, msg.c_str(),
                    ::Smp::Services::ILogger::LMK_Error);
