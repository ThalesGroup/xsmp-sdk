...
std::unique_ptr<Field> Field::Create(::Smp::String8 name,
    ...
    if (type->GetPrimitiveTypeKind() != ::Smp::PrimitiveTypeKind::PTK_None) {
        if (output)
            return std::make_unique<SimpleDataflowField>(name, description,
                    parent, address, type, view, state, input, output);
        else
            return std::make_unique<SimpleField>(name, description, parent,
                    address, type, view, state, input, output);
    }
    ...
    if (const auto *array =
        ...
        if (array->IsSimpleArray()
            ...
            if (output)
                return std::make_unique<SimpleArrayDataflowField>(name,
                        description, parent, address, array, view, state, input,
                        output);
            else
                return std::make_unique<SimpleArrayField>(name, description,
                        parent, address, array, view, state, input, output);
        }
        //normal field
        else {
            if (output)
                return std::make_unique<ArrayDataflowField>(name, description,
                        parent, address, array, view, state, input, output);
            else
                return std::make_unique<ArrayField>(name, description, parent,
                        address, array, view, state, input, output);
        }
    ...
    if (const auto *structure =
        ...
        if (output)
            return std::make_unique<StructureDataflowField>(name, description,
                    parent, address, structure, view, state, input, output);
        else
            return std::make_unique<StructureField>(name, description, parent,
                    address, structure, view, state, input, output);
    }
...
void DataflowField::Push(::Smp::IField *source, ::Smp::IField *target) {
    ...
    // push a simple array field
        ...
        for (size_t i = 0; i < simpleArraySource->GetSize(); ++i)
...
AnonymousArrayField::AnonymousArrayField(::Smp::String8 name,
...
AnonymousSimpleArrayField::AnonymousSimpleArrayField(::Smp::String8 name,
        ::Smp::String8 description, ::Smp::IObject *parent, ::Smp::Int64 count,
        void *address, ::Smp::PrimitiveTypeKind kind, ::Smp::ViewKind view,
        ::Smp::Bool state, ::Smp::Bool input, ::Smp::Bool output) :
        Field(name, description, parent, address, nullptr, view, state, input,
                output), _count(
                count >= 0 ? static_cast<::Smp::UInt64>(count) : 0), _kind(kind) {
...
size_t AnonymousSimpleArrayField::GetItemSize() const {
    switch (_kind) {
...
::Smp::UInt64 AnonymousSimpleArrayField::GetSize() const {
    return _count;
}

::Smp::AnySimple AnonymousSimpleArrayField::GetValue(
        ::Smp::UInt64 index) const {
    if (index >= GetSize())
        ::Xsmp::Exception::throwInvalidArrayIndex(this, index);
...
AnonymousStructureField::AnonymousStructureField(::Smp::String8 name,
        ::Smp::String8 description, ::Smp::IObject *parent,
        ::Smp::Publication::ITypeRegistry *typeRegistry, ::Smp::ViewKind view,
        ::Smp::Bool state) :
        Field(name, description, parent, nullptr, nullptr, view, state, false,
                false), Publication(this, typeRegistry) {
}
...
::Smp::UInt64 ArrayField::GetSize() const {
    return _fields.size();
}
...
SimpleArrayField::SimpleArrayField(::Smp::String8 name,
        ...
        ::Smp::Bool state, ::Smp::Bool input, ::Smp::Bool output) :
        Field(name, description, parent, address, type, view, state, input,
                output), _size(type->GetSize()), _itemType(type->GetItemType()), _itemSize(
                type->GetItemSize()) {
...
::Smp::UInt64 SimpleArrayField::GetSize() const {
    return _size;
}
...
::Smp::Int64 SimpleField::GetSize() const {
    ...
    case ::Smp::PrimitiveTypeKind::PTK_Float64:
        return sizeof(::Smp::Float64);
    case ::Smp::PrimitiveTypeKind::PTK_String8:
        return dynamic_cast<const ::Xsmp::Publication::StringType*>(GetType())->GetLength()
                + 1;
    default:
        ::Xsmp::Exception::throwInvalidPrimitiveType(this, "void", kind);
    }
...
void SimpleField::SetValue(::Smp::AnySimple value) {
    ...
    case ::Smp::PrimitiveTypeKind::PTK_Float64:
        *static_cast<::Smp::Float64*>(GetAddress()) = value;
        break;
    case ::Smp::PrimitiveTypeKind::PTK_String8:
        ::Xsmp::Helper::CopyString(static_cast<char*>(GetAddress()),
                static_cast<::Smp::UInt64>(dynamic_cast<const ::Xsmp::Publication::StringType*>(GetType())->GetLength()
                        + 1), value);
        break;
    default:
        ::Xsmp::Exception::throwInvalidPrimitiveType(this, "void", kind);
...
StructureField::StructureField(::Smp::String8 name, ::Smp::String8 description,
        ::Smp::IObject *parent, void *address,
        const ::Xsmp::Publication::StructureType *type, ::Smp::ViewKind view,
        ::Smp::Bool state, ::Smp::Bool input, ::Smp::Bool output) :
        Field(name, description, parent, address, type, view, state, input,
                output), _fields { "Fields", "", this } {

    for (const auto &field : type->GetFields()) {
        if (auto const *fieldType = type->GetTypeRegistry()->GetType(
                field.uuid))
            _fields.Add(
                    Create(field.name.c_str(), field.description.c_str(), this,
                            static_cast<char*>(address) + field.offset,
                            fieldType, field.view, field.state,
                            field.input || input, field.output || output));
        else
