...
bool Request::isValid(const ::Smp::IObject *sender,
    ...
    // float types
        ...
        if ((floatType->IsMinInclusive() ?
                floatValue < floatType->getMinimum() :
                floatValue <= floatType->getMinimum())
                || (floatType->IsMaxInclusive() ?
...
void Request::init(const std::string &name,
    ...
    else if (auto *structure = dynamic_cast<const StructureType*>(type)) {
        for (auto &field : structure->GetFields()) {
            if (auto const *fieldType = typeRegistry->GetType(field.uuid))
                init(name + "." + field.name, fieldType, typeRegistry);
            else
                ::Xsmp::Exception::throwTypeNotRegistered(_operation,
                        field.uuid);
        }
    }
    // unsupported type
    else {
        ::Xsmp::Exception::throwInvalidAnyType(type,
...
::Smp::Int32 Request::GetParameterIndex(::Smp::String8 parameterName) const {
    if (parameterName) {
        auto it = _indexes.find(parameterName);
        if (it != _indexes.end())
            return it->second;
    }
    return -1;
    ...
    if (!isValid(_operation, _values[static_cast<std::size_t>(index)].second,
        ...
        auto it =
                std::find_if(_indexes.begin(), _indexes.end(),
                        [index](
                                std::map<std::string, ::Smp::Int32>::const_reference e) {
