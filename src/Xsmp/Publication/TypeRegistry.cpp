...
TypeRegistry::TypeRegistry(::Smp::IObject *parent) : _parent(parent) {
  ...
  primitiveTypeKind->AddLiteral("PTK_Bool", "Boolean with true and false.", 2);
  primitiveTypeKind->AddLiteral("PTK_Int8", "8 bit signed integer type.", 3);
  primitiveTypeKind->AddLiteral("PTK_UInt8", "8 bit unsigned integer type.", 4);
  primitiveTypeKind->AddLiteral("PTK_Int16", "16 bit signed integer type.", 5);
  primitiveTypeKind->AddLiteral("PTK_UInt16", "16 bit unsigned integer type.",
                                6);
  primitiveTypeKind->AddLiteral("PTK_Int32", "32 bit signed integer type.", 7);
  primitiveTypeKind->AddLiteral("PTK_UInt32", "32 bit unsigned integer type.",
                                8);
  primitiveTypeKind->AddLiteral("PTK_Int64", "64 bit signed integer type.", 9);
  primitiveTypeKind->AddLiteral("PTK_UInt64", "64 bit unsigned integer type.",
                                10);
  primitiveTypeKind->AddLiteral(
      "PTK_Float32", "32 bit single-precision floating-point type.", 11);
  primitiveTypeKind->AddLiteral(
      "PTK_Float64", "64 bit double-precision floating-point type.", 12);
  primitiveTypeKind->AddLiteral("PTK_Duration", "Duration in nanoseconds.", 13);
  primitiveTypeKind->AddLiteral("PTK_DateTime", "Absolute time in nanoseconds.",
                                14);
  primitiveTypeKind->AddLiteral("PTK_String8", "8 bit character string.", 15);

  AddIntegerType("EventId", // Name
                 "Identifier of global event of scheduler or event manager "
                 "service.",                               // description
  ...
  auto *viewKind = AddEnumerationType(
      "ViewKind", // name
...
The simulation infrastructure may support the selection of different user roles, in which case the "Debug" and the "Expert" role must also be supported as described.)", // description
  ...
  viewKind->AddLiteral(
      "VK_None",
      "The element is not made visible to the user (this is the default).", 0);
...
The element is not visible to end users. If the simulation infrastructure supports the selection of different user roles, then the element shall be visible to "Debug" and "Expert" users.)",
  ...
  parameterDirectionKind->AddLiteral("PDK_InOut",
                                     "The parameter must be specified on call, "
                                     "and may be changed by the operation.",
                                     2);
  parameterDirectionKind->AddLiteral(
  ...
  componentStateKind->AddLiteral(
      "CSK_Publishing",
      "In Publishing state, the component is allowed to publish features. This "
      "includes publication of fields, operations and properties. In addition, "
      "the component is allowed to create other components."
  ...
  componentStateKind->AddLiteral(
      ...
      "configuration may be done by external components, or internally by the "
  ...
  simulatorStateKind->AddLiteral(
      "SSK_Building",
      "In Building state, the component hierarchy is created. This is done by "
      "an external component, not by the simulator."
      "This state is entered automatically after the simulation environment "
      "has performed its initialisation."
      "In this state, Publish() and Configure() can be called any time to call "
      "the corresponding Publish() and Configure() operations of each "
      "component."
      "This state is left with the Connect() state transition method.",
      0);
  simulatorStateKind->AddLiteral(
      "SSK_Connecting",
      "In Connecting state, the simulation environment traverses the component "
      "hierarchy and calls the Connect() method of each component."
      "This state is entered using the Connect() state transition."
      "After connecting all components to the simulator, an automatic state "
      "transition to the Initialising state is performed.",
      1);
  simulatorStateKind->AddLiteral(
      "SSK_Initialising",
      "In Initialising state, the simulation environment executes all "
      "initialisation entry points in the order they have been added to the "
      "simulator using the AddInitEntryPoint() method."
      "This state is either entered automatically after the simulation "
      "environment has connected all models to the simulator, or manually from "
      "Standby state using the Initialise() state transition."
      "After calling all initialisation entry points, an automatic state "
      "transition to the Standby state is performed.",
      2);
  simulatorStateKind->AddLiteral(
      "SSK_Standby",
      "In Standby state, the simulation environment (namely the Time Keeper "
      "Service) does not progress simulation time. Only entry points "
      "registered relative to Zulu time are executed."
      "This state is entered automatically from the Initialising, Storing, and "
      "Restoring states, or manually from the Executing state using the Hold() "
      "state transition."
  ...
  simulatorStateKind->AddLiteral(
      "SSK_Executing",
      "In Executing state, the simulation environment (namely the Time Keeper "
      "Service) does progress simulation time. Entry points registered with "
      "any of the available time kinds are executed."
      "This state is entered using the Run() state transition."
      "This state is left using the Hold() state transition.",
      4);
  simulatorStateKind->AddLiteral(
      "SSK_Storing",
      "In Storing state, the simulation environment first stores the values of "
      "all fields published with the State attribute to storage (typically a "
      "file). Afterwards, the Store() method of all components (Models and "
      "Services) implementing the optional IPersist interface is called, to "
      "allow custom storing of additional information. While in this state, "
      "fields published with the State attribute must not be modified by the "
      "models, to ensure that a consistent set of field values is stored."
      "This state is entered using the Store() state transition."
      "After storing the simulator state, an automatic state transition to the "
      "Standby state is performed.",
      5);
  simulatorStateKind->AddLiteral(
      "SSK_Restoring",
      "In Restoring state, the simulation environment first restores the "
      "values of all fields published with the State attribute from storage. "
      "Afterwards, the Restore() method of all components implementing the "
      "optional IPersist interface is called, to allow custom restoring of "
      "additional information. While in this state, fields published with the "
      "State attribute must not be modified by the models, to ensure that a "
      "consistent set of field values is restored."
      "This state is entered using the Restore() state transition."
      "After restoring the simulator state, an automatic state transition to "
      "the Standby state is performed.",
      6);
  simulatorStateKind->AddLiteral(
      "SSK_Reconnecting",
      "In Reconnecting state, the simulation environment makes sure that "
      "models that have been added to the simulator after leaving the Building "
      "state are properly published, configured and connected."
      "This state is entered using the Reconnect() state transition."
      "After connecting all new models, an automatic state transition to the "
      "Standby state is performed.",
      7);
  simulatorStateKind->AddLiteral(
      "SSK_Exiting",
      "In Exiting state, the simulation environment is properly terminating a "
      "running simulation."
      "This state is entered using the Exit() state transition. After exiting, "
...
::Smp::String8 TypeRegistry::GetName() const { return "TypeRegistry"; }
...
::Smp::Publication::IType *
TypeRegistry::AddFloatType(::Smp::String8 name, ::Smp::String8 description,
