...
template <typename Tp, typename Up, typename = void> struct _cmp_equal_helper {
...
struct _cmp_equal_helper<
    Tp, Up, std::enable_if_t<std::is_signed_v<Tp> == std::is_signed_v<Up>>> {
  static constexpr bool apply(Tp _t, Up _u) noexcept { return _t == _u; }
};
template <typename Tp, typename Up>
struct _cmp_equal_helper<
    Tp, Up, std::enable_if_t<std::is_signed_v<Tp> && !std::is_signed_v<Up>>> {
...
template <typename Tp, typename Up, typename = void> struct _cmp_less_helper {
  static constexpr bool apply(Tp _t, Up _u) noexcept {
    return _u < 0 ? false : _t < static_cast<std::make_unsigned_t<Up>>(_u);
  }
};
...
template <typename Up, typename Tp> constexpr bool in_range(Tp _t) noexcept {
