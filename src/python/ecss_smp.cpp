...
static inline const TypeHierarchy IObjectHierarchy =
    TypeHierarchy::template of<::Smp::IObject>({
        ...
        TypeHierarchy::template of<::Smp::IComponent>({
            // IComponent
            TypeHierarchy::template of<::Smp::IAggregate>(),

            TypeHierarchy::template of<::Smp::IDynamicInvocation>(),

            TypeHierarchy::template of<::Smp::IEventConsumer>(),

            TypeHierarchy::template of<::Smp::IEventProvider>(),

            TypeHierarchy::template of<::Smp::ILinkingComponent>(),

            TypeHierarchy::template of<::Smp::IModel>({
                // IModel
                TypeHierarchy::template of<::Smp::IFallibleModel>(),

            }),

            TypeHierarchy::template of<::Smp::IService>({
                // IService
                TypeHierarchy::template of<::Smp::Services::IEventManager>(),

                TypeHierarchy::template of<::Smp::Services::ILinkRegistry>(),

                TypeHierarchy::template of<::Smp::Services::ILogger>(),

                TypeHierarchy::template of<::Smp::Services::IResolver>(),

                TypeHierarchy::template of<::Smp::Services::IScheduler>(),

                TypeHierarchy::template of<::Smp::Services::ITimeKeeper>(),
        ...
        TypeHierarchy::template of<::Smp::IPersist>({
            // IPersist
            TypeHierarchy::template of<::Smp::IFailure>(),

            TypeHierarchy::template of<::Smp::IField>({
                // IField
                TypeHierarchy::template of<::Smp::IArrayField>(),

                TypeHierarchy::template of<::Smp::ISimpleArrayField>(),

                TypeHierarchy::template of<::Smp::ISimpleField>({
                    // ISimpleField
                    TypeHierarchy::template of<::Smp::IForcibleField>(),
        ...
        TypeHierarchy::template of<::Smp::Publication::IType>({
            ...
            TypeHierarchy::template of<::Smp::Publication::IStructureType>({
                // IStructureType
                TypeHierarchy::template of<::Smp::Publication::IClassType>(),
...
public:
  ...
  static void init_instance(py::detail::instance *inst,
    ...
    auto v_h = py::detail::value_and_holder(
        inst, py::detail::get_type_info(typeid(type)), 0, 0);
...
private:
  static void
  init_holder_from_existing(const py::detail::value_and_holder &v_h,
  ...
  /// object, if possible
  static void
  init_holder(const py::detail::instance *inst,
...
PYBIND11_MODULE(ecss_smp, ecss_smp) {
  ...
  auto smp =
      ecss_smp.def_submodule("Smp", "SMP standard types and interfaces.");

  const auto &Exception = py::register_exception<::Smp::Exception>(
      smp, "Exception", PyExc_RuntimeError);
  Exception.doc() =
...
@remarks This may e.g. be if there is no disk space left.)";
  ...
  SMP_EXCEPTION(DuplicateName).doc() =
      ...
      "of objects, "
      "which have to have unique names, but another object with the same name "
      "does exist already in this collection. "
  ...
  SMP_EXCEPTION(DuplicateUuid).doc() =
      ...
      "that has already been used to register "
      "another (or the same) factory. This would lead to duplicate "
...
#undef SMP_EXCEPTION
  auto publication =
      smp.def_submodule("Publication", "Namespace for publication");

  auto services =
      smp.def_submodule("Services", "Namespace for simulation services");
  ...
  py::register_exception<::Smp::Publication::DuplicateLiteral>(
      publication, "DuplicateLiteral", Exception)
      .doc() =
      "This exception is raised when trying to add a literal to an enumeration "
      "using a value that has been used for another literal before.";

  py::register_exception<::Smp::Publication::InvalidPrimitiveType>(
      publication, "InvalidPrimitiveType", Exception)
      .doc() =
  ...
  py::register_exception<::Smp::Publication::TypeAlreadyRegistered>(
      ...
      .doc() = "This exception is raised when trying to register a type with a "
               "Uuid that has already been registered.";

  py::register_exception<::Smp::Publication::TypeNotRegistered>(
      publication, "TypeNotRegistered", Exception)
      .doc() = "This exception is raised when trying to publish a feature with "
               "a type Uuid that has not been registered.";

  py::register_exception<::Smp::Services::EntryPointAlreadySubscribed>(
      services, "EntryPointAlreadySubscribed", Exception)
      .doc() = "This exception is raised when trying to subscribe an entry "
               "point to an event that is already subscribed.";

  py::register_exception<::Smp::Services::EntryPointNotSubscribed>(
      services, "EntryPointNotSubscribed", Exception)
      .doc() = "This exception is raised when trying to unsubscribe an entry "
               "point from an event that is not subscribed to it.";

  py::register_exception<::Smp::Services::InvalidCycleTime>(
      services, "InvalidCycleTime", Exception)
      .doc() = "This exception is thrown by one of the AddEvent() methods of "
               "the scheduler when the event is a cyclic event "
               "(i.e. repeat is not 0), but the cycle time specified is not a "
               "positive duration.";

  py::register_exception<::Smp::Services::InvalidEventId>(
      services, "InvalidEventId", Exception)
      .doc() =
      "This exception is raised when an invalid event id is provided, e.g. "
      "when calling Subscribe(), Unsubscribe() "
      "or Emit() of the Event Manager (using an invalid global event id), or "
      "when calling SetEventSimulationTime(), "
      "SetEventMissionTime(), SetEventEpochTime(), SetEventZuluTime(), "
      "SetEventCycleTime(), SetEventCount() or "
      "RemoveEvent() of the Scheduler (using an invalid scheduler event id).";

  py::register_exception<::Smp::Services::InvalidEventName>(
      services, "InvalidEventName", Exception)
      .doc() = "This exception is thrown by the QueryEventId() method of the "
               "event manager when an empty event name has been provided.";

  py::register_exception<::Smp::Services::InvalidEventTime>(
      services, "InvalidEventTime", Exception)
      .doc() = "This exception is thrown by one of the AddEvent() methods of "
               "the scheduler when the time specified for the first execution "
               "of the event is in the past.";

  py::register_exception<::Smp::Services::InvalidSimulationTime>(
      services, "InvalidSimulationTime", Exception)
      .doc() =
      "This exception is thrown by SetSimulationTime if the new simulation "
      "time is not between the current simulation "
