...
inline py::object callOperation(::Smp::IOperation &self, const py::args &args,
    ...
    auto request = std::unique_ptr<::Smp::IRequest,
            std::function<void(::Smp::IRequest*)>>(self.CreateRequest(),
            [&self](::Smp::IRequest *r) {
                self.DeleteRequest(r);
            });
    if (!request)
        throw py::type_error(
                std::string(self.GetName()) + "() is not invokable");

    const auto *parameters = self.GetParameters();
    if (args.size() > parameters->size())
        throw py::type_error(
                std::string(self.GetName()) + "() takes "
    ...
    if (const auto *returnParameter = self.GetReturnParameter()) {
        result.attr(returnParameter->GetName()) = convert(
    ...
    for (const auto *parameter : *parameters) {
        if (parameter->GetDirection()
            ...
            result.attr(parameter->GetName()) = convert(
