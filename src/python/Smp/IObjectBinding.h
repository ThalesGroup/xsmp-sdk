...
inline void RegisterIObject(const py::module_ &m) {
  py::class_<::Smp::IObject>(m, "IObject")
      ...
      // Returning a py::object rather than a Smp::IObject* is deliberate: the
      // resolved child can be a field, a container, an entry point, ... and
      // reporting IObject would make every dynamically resolved attribute
      // useless to a static analyser. `typing.Any` is the honest annotation;
      // the precise types come from the assembly stub generated by
      // generate_python_type_hints().
      .def(
          "__getattr__",
          [](::Smp::IObject &self, ::Smp::String8 name) -> py::object {
            if (auto *child = ::Xsmp::Helper::Resolve(&self, name)) {
              return py::cast(child, py::return_value_policy::reference);
            }
            throw py::attribute_error(name);
          },
          // keep_alive<0, 1> reproduces
          // return_value_policy::reference_internal, which cannot be expressed
          // on a lambda casting its result by hand
      ...
      // see __getattr__ above: the resolved type is only known dynamically
      .def(
          "__getitem__",
          [](::Smp::IObject &self, ::Smp::String8 name) -> py::object {
            if (auto *child = ::Xsmp::Helper::Resolve(&self, name)) {
