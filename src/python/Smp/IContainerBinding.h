...
A return value of -1 indicates that the collection has no upper limit.)")
      ...
      .def(
          ...
          [](const ::Smp::IContainer &self, ::Smp::Int64 index) {
            if (auto *cmp = self.GetComponents()->at(GetIndex(
                    index, static_cast<::Smp::UInt64>(self.GetCount()))))
              return cmp;
      ...
      .def(
          "__getitem__",
          [](const ::Smp::IContainer &self, ::Smp::String8 name) {
      ...
      .def(
          "__delitem__",
          [](::Smp::IContainer &self, ::Smp::String8 name) {
            if (auto *cmp = self.GetComponents()->at(name))
              self.DeleteComponent(cmp);
            else
              throw py::key_error(name);
...
The returned component may be null if no child with the given name could be found.)")
      ...
      .def("AddComponent", &::Smp::IContainer::AddComponent,
           py::arg("component"),
