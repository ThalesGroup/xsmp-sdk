...
TEST(ExceptionTest, FieldAlreadyConnected) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot connect a target field to a data flow field that is already connected");
        EXPECT_EQ(e.GetSender(), &sender);
...
TEST(ExceptionTest, InvalidTarget) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "The type of the target '<null>.target' is not compatible with '<null>.source'.");
...
TEST(ExceptionTest, ContainerFull) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot add a component to a container that is full, i.e. where the Count has reached the Upper limit");
...
TEST(ExceptionTest, DuplicateName) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to add an object named 'test' in '<null>.name' collection already containing an object with this name.");
...
TEST(ExceptionTest, NotContained) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "This exception is thrown when trying to delete a component from a container which was not contained before");
        EXPECT_EQ(e.GetSender(), &ctn);
        EXPECT_STREQ(e.GetMessage(),
                "Cannot delete component '<null>/cmp' from the container '<null>.parent.ctn'.");
...
TEST(ExceptionTest, CannotDelete) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &ctn);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to delete <null>/cmp component from <null>.parent.ctn container, but the number of contained components is lower than or equal to the Lower limit: 42");
...
TEST(ExceptionTest, InvalidComponentState) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "This exception is raised by a component when one of its methods is called in an invalid state");
        EXPECT_EQ(e.GetSender(), &cmp);
        EXPECT_STREQ(e.GetMessage(),
                "The ComponentState of <null>/cmp is in Connected state instead of Created state.");
...
TEST(ExceptionTest, NotReferenced) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &ctn);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to remove <null>/cmp component from the reference <null>.parent.ctn which was not referenced before.");
...
TEST(ExceptionTest, ReferenceFull) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot add a component to a reference that is full, i.e. where the Count has reached the Upper limit");
        EXPECT_EQ(e.GetSender(), &ctn);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to add a component to <null>.parent.ctn that is full (max size: 42)");
...
TEST(ExceptionTest, CannotRemove) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &ctn);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to remove <null>/cmp component from <null>.parent.ctn reference, but the number of referenced components is lower than or equal to the Lower limit: 42");
...
TEST(ExceptionTest, InvalidEventSink) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot subscribe an event sink to an event source that has a different event type");
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to subscribe <null>.parent.eso1 event source to <null>.parent.esi1 event sink which has a different type.");
...
TEST(ExceptionTest, EventSinkAlreadySubscribed) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot subscribe an event sink to an event source that is already subscribed");
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to subscribe <null>.parent.eso1 event source to <null>.parent.esi1 event sink that is already subscribed.");
...
TEST(ExceptionTest, EventSinkNotSubscribed) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot unsubscribe an event sink from an event source that is not subscribed to it");
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to unsubscribe <null>.parent.esi1 event sink from <null>.parent.eso1 event source that is not subscribed to it.");
...
TEST(ExceptionTest, InvalidOperationName) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to invoke a method that does not exist, or that does not support dynamic invocation in operation: op");
...
TEST(ExceptionTest, InvalidParameterCount) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Tried to invoke operation <null>.op with 42 parameter(s) instead of 0.");
...
TEST(ExceptionTest, InvalidParameterType) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "In operation op, tried to pass parameter parameter which is of type Bool instead of expected type Float32");
...
TEST(ExceptionTest, InvalidArrayIndex) {
    ...
    auto *type =
            dynamic_cast<Xsmp::Publication::ArrayType*>(registry.AddArrayType(
                    "type", "", Smp::Uuid { }, Smp::Uuids::Uuid_Bool,
                    sizeof(bool), 10, false));

    Xsmp::Publication::ArrayField sender { "field", "", nullptr, nullptr, type,
            Smp::ViewKind::VK_All, true, false, false };
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "The index '42' is negative or exceeds the array size '10' of '<null>.field'.");
    ...
    Xsmp::Publication::SimpleArrayField simple { "field", "", nullptr, nullptr,
            type, Smp::ViewKind::VK_All, true, false, false };
    ...
    try {
        ...
        EXPECT_STREQ(e.GetMessage(),
                "The index '-1' is negative or exceeds the array size '10' of '<null>.field'.");
...
TEST(ExceptionTest, InvalidFieldValue) {
    ...
    auto invalidValue = Smp::AnySimple { ::Smp::PrimitiveTypeKind::PTK_Float32,
            42. };
    try {
        ...
        EXPECT_STREQ(e.GetMessage(),
                "Cannot assign value '42f32' to the field <null>.field of Bool's type.");
...
TEST(ExceptionTest, InvalidArrayValue) {
    ...
    auto *type =
            dynamic_cast<Xsmp::Publication::ArrayType*>(registry.AddArrayType(
                    "Test", "", Smp::Uuid { }, Smp::Uuids::Uuid_Bool,
                    sizeof(bool), 10, false));

    Xsmp::Publication::SimpleArrayField sender { "field", "", nullptr, nullptr,
            type, Smp::ViewKind::VK_All, true, false, false };
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "Cannot assign value '42f32' to the field <null>.field[4] of Test's type.");
...
TEST(ExceptionTest, InvalidEventId) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "This exception is raised when an invalid event id is provided, e.g.when calling Subscribe(),"
                        " Unsubscribe() or Emit() of the Event Manager (using an invalid global event id), or when calling "
                        "SetEventSimulationTime(), SetEventMissionTime(), SetEventEpochTime(), SetEventZuluTime(), SetEventCycleTime(),"
...
TEST(ExceptionTest, InvalidCycleTime) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "This exception is thrown by one of the AddEvent() methods of the scheduler when the event "
                        "is a cyclic event (i.e. repeat is not 0),  but the cycle time specified is not a positive duration");
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
...
TEST(ExceptionTest, InvalidEventTime) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "The event time '00:00:00.000001000' is before the current time '00:00:00.000002000'.");
...
TEST(ExceptionTest, TypeNotRegistered) {
    ...
    try {
        ...
        EXPECT_STREQ(e.GetDescription(),
                "Cannot publish a feature with a type Uuid that has not been registered");
        EXPECT_EQ(e.GetSender(), &sender);
        EXPECT_STREQ(e.GetMessage(),
                "The Uuid '00000000-0001-0002-0003-000000000004' has not been registered.");
...
TEST(ExceptionTest, InvalidParameterIndex) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "The Operation '<null>.op' has '8' parameter(s). Cannot access to parameter at index '42'.");
...
TEST(ExceptionTest, InvalidParameterValue) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "The value 'true' is invalid for parameter 'param' in Operation '<null>.op'.");
...
TEST(ExceptionTest, InvalidFieldType) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "The field type of '<null>.op' is invalid.");
...
TEST(ExceptionTest, DuplicateUuid) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "The Uuid '00000001-0002-0003-0004-000000000005' of Factory 'M1Duplicate' collides with the existing factory 'M1'.");
...
TEST(ExceptionTest, InvalidSimulationTime) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "The simulation time '4' is invalid. Expecting a value between '1' and '3'.");
...
TEST(ExceptionTest, TypeAlreadyRegistered) {
    ...
    try {
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Cannot register 'OtherType' with Uuid '00000000-0000-0000-0000-000000000000'. This Uuid has already been registered.");
...
TEST(ExceptionTest, InvalidSimulatorState) {
    ...
    try {
        throwInvalidSimulatorState(&parent,
        ...
        EXPECT_EQ(e.GetSender(), &parent);
        EXPECT_STREQ(e.GetMessage(),
                "Cannot performs an operation while the simulator is in 'Executing' state.");
