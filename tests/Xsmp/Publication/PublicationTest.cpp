...
TEST(PublicationTest, PublishField) {
    ...
    auto *stateEnumField =
            dynamic_cast<Smp::ISimpleField*>(publication.GetField(
                    "stateEnumField"));
    ...
    EXPECT_EQ(Enum, 1);
    EXPECT_THROW(stateEnumField->SetValue( { Smp::PrimitiveTypeKind::PTK_Int32,
            42 }), Smp::InvalidFieldValue);
...
void TestPublishSimpleField(Smp::PrimitiveTypeKind kind, Smp::ViewKind view,
    ...
    publication.PublishField("state", "description", &state, view, true, false,
            false);
    auto *stateField = dynamic_cast<Smp::ISimpleField*>(publication.GetField(
            "state"));
    ...
    publication.PublishField("input", "description", &input, view, false, true,
            false);
    auto *inputField = dynamic_cast<Smp::ISimpleField*>(publication.GetField(
            "input"));
    ASSERT_TRUE(inputField);
    EXPECT_TRUE(inputField->IsInput());

    T output = check_Value;
    publication.PublishField("output", "description", &output, view, false,
            false, true);
    auto *outputField = dynamic_cast<Smp::ISimpleField*>(publication.GetField(
            "output"));
    ...
    EXPECT_THROW(dataflowField->Connect(inputField),
...
TEST(PublicationTest, PublishSimpleField) {
    ...
    Publication publication { &component, &registry };
    EXPECT_THROW(
            publication.PublishField("test", "", nullptr, { },
...
void TestPublishSimpleArray(Smp::PrimitiveTypeKind kind, Smp::ViewKind view,
    ...
    publication.PublishArray("state", "description", 2, &state, kind, view,
            true, false, false);
    auto *stateField =
    ...
    publication.PublishArray("transient", "description", 2, &state, kind, view,
            false, false, false);
    auto *transientField =
            dynamic_cast<Smp::ISimpleArrayField*>(publication.GetField(
                    "transient"));
    ASSERT_TRUE(transientField);
    EXPECT_FALSE(transientField->IsState());

    Xsmp::Array<T, 2> input = { default_value, default_value };
    publication.PublishArray("input", "description", 2, &input, kind, view,
            true, true, false);
    auto *inputField =
...
TEST(PublicationTest, PublishSimpleArray) {
    ...
    EXPECT_THROW(
            TestPublishSimpleArray<Smp::UInt64>(
                    Smp::PrimitiveTypeKind::PTK_None, Smp::ViewKind::VK_None, 0,
                    100), Smp::InvalidFieldType);

    EXPECT_THROW(
            TestPublishSimpleArray<Smp::String8>(
                    Smp::PrimitiveTypeKind::PTK_String8, Smp::ViewKind::VK_None,
                    "", "test"), Smp::InvalidFieldType);
}
...
TEST(PublicationTest, PublishArray) {
    ...
    auto *publishedField = publication.PublishArray("name", "desc",
            Smp::ViewKind::VK_None, true);

    ASSERT_TRUE(publishedField);

    auto *arrayField = dynamic_cast<Smp::IArrayField*>(publication.GetField(
            "name"));
...
TEST(PublicationTest, PublishStructure) {
    ...
    auto *structureField = publication.PublishStructure("name", "",
            Smp::ViewKind::VK_None, true);
...
TEST(PublicationTest, PublishOperation) {
    ...
    auto *operation = publication.PublishOperation("operation", "",
    ...
    EXPECT_EQ(operation,
            publication.PublishOperation("operation", "desc",
    ...
    operation->PublishParameter("returnValue", "bool", Smp::Uuids::Uuid_Bool,
    ...
    EXPECT_THROW(
            operation->PublishParameter("returnValue2", "",
                    Smp::Uuids::Uuid_Bool,
                    Smp::Publication::ParameterDirectionKind::PDK_Return),
            Smp::Exception);

    operation->PublishParameter("param2", "desc", Smp::Uuids::Uuid_Float32,
    ...
    operation->PublishParameter("param3", "desc", Smp::Uuids::Uuid_Int64,
    ...
    EXPECT_THROW(
            operation->PublishParameter("param4", "", { },
                    Smp::Publication::ParameterDirectionKind::PDK_Out),
            Smp::Publication::TypeNotRegistered);
}
...
TEST(PublicationTest, PublishProperty) {
    ...
    EXPECT_THROW(
            publication.PublishProperty("property", "", { },
    ...
    publication.PublishProperty("property", "desc", Smp::Uuids::Uuid_Int8,
            ::Smp::AccessKind::AK_ReadWrite, Smp::ViewKind::VK_Debug);
