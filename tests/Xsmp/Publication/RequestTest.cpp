...
TEST(RequestTest, VoidOperation) {
    ...
    EXPECT_THROW(request->SetParameterValue(-1, { }),
            Smp::InvalidParameterIndex);
    EXPECT_THROW(request->SetParameterValue(0, { }),
            Smp::InvalidParameterIndex);
...
TEST(RequestTest, EnumOperation) {
    ...
    auto *op = publication.PublishOperation("operation", "",
    ...
    Smp::AnySimple value = { Smp::PrimitiveTypeKind::PTK_Int32,
    ...
    EXPECT_THROW(request->SetReturnValue(invalidValue),
...
TEST(RequestTest, StringOperation) {
    ...
    Smp::AnySimple invalidValue =
            { Smp::PrimitiveTypeKind::PTK_String8, "12345" };
    EXPECT_THROW(request->SetReturnValue(invalidValue),
            Smp::InvalidReturnValue);
    publication.DeleteRequest(request);
...
TEST(RequestTest, IntOperation) {
    ...
    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Int32,
            11 }), Smp::InvalidReturnValue);

    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Int32,
            -1 }), Smp::InvalidReturnValue);
    publication.DeleteRequest(request);
...
TEST(RequestTest, FloatOperation) {
    ...
    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Float64,
            11. }), Smp::InvalidReturnValue);

    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Float64,
            -1. }), Smp::InvalidReturnValue);

    EXPECT_NO_THROW(request->SetReturnValue( {
            Smp::PrimitiveTypeKind::PTK_Float64, 10. }));

    EXPECT_NO_THROW(request->SetReturnValue( {
            Smp::PrimitiveTypeKind::PTK_Float64, 0. }));
...
TEST(RequestTest, FloatExclusiveOperation) {
    ...
    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Float64,
            10. }), Smp::InvalidReturnValue);

    EXPECT_THROW(request->SetReturnValue( { Smp::PrimitiveTypeKind::PTK_Float64,
            0. }), Smp::InvalidReturnValue);

    EXPECT_NO_THROW(request->SetReturnValue( {
            Smp::PrimitiveTypeKind::PTK_Float64, 9.9999 }));

    EXPECT_NO_THROW(request->SetReturnValue( {
            Smp::PrimitiveTypeKind::PTK_Float64, 0.00001 }));
